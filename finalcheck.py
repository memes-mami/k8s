...
for chunk_df in chunks:
    ...
    for i, node in enumerate(ranked_nodes_electre, start=1):
        print(f"{i}. {node}")
    node_name = ranked_nodes_electre[1]
    ...
    print(f"the selected node to checkpoint :{ranked_nodes_electre[1]}")
    start_time = time.time()
    run_bash_script(bash_script_path, [node_name,picked_zookeeper_pod])
    duration = time.time() - start_time
    print(f"Time Duration for the first script: {duration} seconds")

    bash_s2 = 'checkr.sh' 
    print(f"the selected pod to checkpoint :{ranked_nodes_electre[-1]}")
    run_bash_script(bash_s2, [ranked_nodes_electre[-1]])
    duration = time.time() - start_time
    print(f"Time Duration for the second script: {duration} seconds")    
