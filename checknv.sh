...
if [ "$CPU_USAGE" -gt "$CPU_THRESHOLD" ] && [ "$MEMORY_USAGE" -gt "$MEMORY_THRESHOLD" ]; then
    echo "yes"
    bash finding_n_v_n.sh "$check_node"
    bash finding_n_v_n.sh "$WORKER_NODE"
    # Run restorez.sh with the provided worker node name as an argument
    start_time=$(date +%s.%N)
    bash restorenv.sh "$WORKER_NODE" "$check_node"
    end_time=$(date +%s.%N)
    execution_time=$(echo "$end_time - $start_time" | bc)
    echo "$check_node,$WORKER_NODE,$execution_time" >> restore_n_v.csv
    python_script="delete.py"
    python3 "$python_script" "$pod"
    bash changetnv.sh "$check_node"
else
    echo "no"
fi
