...
def get_zookeeper_pods_on_node(node_name):
    ...
    try:
        ...
        # Filter pods that are running on the specified node and labeled with 'app=zookeeper'
        zookeeper_pods = [pod for pod in pods_list.items
                          if pod.spec.node_name == node_name and 'app' in pod.metadata.labels and pod.metadata.labels['app'] == 'zookeeper']
...
if __name__ == "__main__":
    ...
    for i in range(0, len(df), chunk_size):
        ...
        if len(zookeeper_pods) > 0:
            ...
            # Provide an empty string as an argument
            picked_zookeeper_pod = client.V1Pod(metadata=client.V1ObjectMeta(name=''))
        # Provide arguments as a list
        script_arguments = [ranked_workers[0], picked_zookeeper_pod.metadata.name if picked_zookeeper_pod.metadata.name else ""]
