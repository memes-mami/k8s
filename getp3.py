...
def get_total_resources(metrics):
    ...
    try:
        ...
        return str(total_resources)  # Convert to string for accurate sorting
    except ValueError:
        return None
...
def main():
    ...
    if nginx_pods:
        ...
        # Sort pods based on total resources (converted to string for accurate sorting)
        sorted_pods = sorted(pod_resources, key=lambda x: x[1], reverse=False)
