...
#    ranked_nodes_topsis = chunk_df.sort_values(by='Closeness_TOPSIS', ascending=False)['Node'].tolist()
    ...
    bash_s2 = 'restoren2.sh'
    print(f"the selected pod to checkpoint :{ranked_nodes_electre[-1]}")
    # Run the Bash script and capture the output
    output = subprocess.check_output(["bash", bash_s2,ranked_nodes_electre[-1]], text=True)
    execution_time_line = output.splitlines()[-1]
    print("finallllllllllllllle Execution time:", execution_time_line)
