...
if [ "$CPU_USAGE" -gt "$CPU_THRESHOLD" ] || [ "$MEMORY_USAGE" -gt "$MEMORY_THRESHOLD" ]; then
    echo "yes"
    bash finding_n_t_n.sh "$node_name"
    bash finding_n_t_n.sh "$WORKER_NODE"
    # Run restorez.sh with the provided worker node name as an argument
    start_time=$(date +%s.%N)
    bash restorent.sh "$WORKER_NODE" "$node_name"
    end_time=$(date +%s.%N)
    execution_time=$(echo "$end_time - $start_time" | bc)
    echo "$node_name,$WORKER_NODE,$execution_time" >> restore_n_t.csv
else
    echo "no"
fi
